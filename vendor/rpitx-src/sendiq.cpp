/*
 * NOTICE - MODIFIED FROM UPSTREAM (webtx project)
 *
 * This file is derived from F5OEO/rpitx's src/sendiq.cpp and has been
 * MODIFIED by the webtx project (https://github.com/ta1rjv/rpitx_webtx).
 * The modification adds two optional command-line flags, -b (read burst
 * size) and -F (DMA FIFO size), so this vendored sendiq can be run at
 * lower latency without touching librpitx's shared FIFO-pacing policy.
 * When neither flag is passed, all defaults and behavior are UNCHANGED
 * from upstream.
 *
 * See patches/sendiq-lowlatency.patch (the same change, packaged as an
 * optional patch for a separate real upstream rpitx install) and
 * vendor/NOTICE.md (vendoring rationale and license details) at the root
 * of the webtx repository. This notice is carried per GPLv3 section 5(a),
 * which requires modified files to carry prominent notices stating that
 * they have been changed and the date of any change (see vendor/NOTICE.md
 * and docs/DECISIONS.md ADR-010 for the exact vendored commit and date).
 *
 * Everything else in this vendored source tree (vendor/rpitx-src/) is an
 * unmodified copy of upstream; this is the only file that differs.
 *
 * Original license: GNU General Public License v3.0 (see
 * vendor/rpitx-src/LICENSE, the exact text distributed with upstream).
